    ...
    if(graphState.GraphSettings.type === 'student'){
        ...
        return(
            <div style={{display:'flex', flexDirection:'column', backgroundColor:'#1d333d', backgroundPosition:'center', backgroundRepeat:'no-repeat', backgroundSize:'cover', width:'100%', boxSizing:'border-box', padding:'0px 30px 30px 30px', minHeight:'100vh', justifyContent:'space-between'}}>
                <div style={{display:'flex', backgroundPosition:'center', backgroundRepeat:'no-repeat', backgroundSize:'cover', width:'100%', flexDirection:'column', alignItems:'center', height:'650px'}}>
                        ...
                        {graphState.GraphSettings.data.topThree.map((student:IStudentProps,key) => {
                            return(
                                <Banner key={key} keyPos={key} type='Parent' backgroundColor={student.houseColor} size={key === 0? 'Medium' : key === 1? 'Large' : 'Small'}>
                                    ...
                                    <BannerContent keyPos={key} studentName={student.studentForename} studentReg={student.studentReg} studentPoints={student.points} backgroundColor={student.houseColor} darkBackgroundColor={student.darkerHouseColor} studentPhoto={student.blob? student.blob : undefined}/>
                                </Banner>
                            )
                ...
                <div style={{display:'flex', flexDirection:'column', width:'100%', gap:'40px'}}>
                    <div className='roleOfHonourHeadingContainer'>
                        <img style={{width:'200px'}} src={crown} alt='Crown'/>
                        <h1 style={{color:'#e1b164', margin:'0px', fontFamily: 'Bebas Neue", sans-serif', fontWeight:'600', fontSize:'60px'}}>THIS WEEK'S</h1>
                                    ...
                                    <TableRow key={key} keyPos={key}>
                                        ...
                                        <TableColumn width='80%'>
                                            <div style={{display:'flex', flexDirection:'row', gap:'25px', alignItems:'center'}}>
                                                <div id='circle-photo' style={{display:'flex', width:'70px', height:'70px', backgroundColor:'white', borderRadius:'200px', alignItems:'center', justifyContent:'center', overflow:'hidden', border:'solid 3px #e1b164', flexShrink:'0'}}>
                                                    <div id='photo' style={{display:'flex', opacity:`${student.blob? '1' : '0.25'}`, width:`${student.blob? '100px' : '50px'}`, height:`${student.blob? '100px' : '50px'}`, backgroundImage:`url(${student.blob? student.blob : user})`, backgroundRepeat:'no-repeat', backgroundSize:'contain', backgroundPosition:'center'}}>
                                                        
                                                    </div>
                                                </div>
                                                <p style={{fontSize:'40px', margin:'0px', textOverflow:'ellipsis', overflow:'hidden', textWrap:'nowrap'}}>
                                                    <span style={{fontWeight:'bold'}}>{student.studentForename + ' ' + student.studentSurname}</span>
                                                    {' - ' + student.studentReg}
                                                </p>
                                            </div>
                                        </TableColumn>
